    ...
    companion object {
        ...
        fun getFileLines(file: File?): Int {
            ...
            try {
                ...
                var readChars: Int = input.read(buffer, 0, 1024)
                if (LINE_SEP.endsWith("\n")) {
                    while (input.read(buffer, 0, 1024).let { readChars = it; it != -1 }) {
                        (0 until readChars)
                                .filter { buffer[it].toChar() == '\n' }
                                .forEach { ++count }
                    }
                } else {
                    while (input.read(buffer, 0, 1024).let { readChars = it; it != -1}) {
                        (0 until readChars)
                                .filter { buffer[it].toChar() == '\r' }
                                .forEach { ++count }
                    }
                }
            } catch (e: IOException) {
