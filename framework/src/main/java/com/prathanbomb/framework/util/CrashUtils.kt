...
class CrashUtils private constructor() {
    ...
    companion object {
         ...
         */
        @JvmOverloads
        fun init(crashDir: String = ""): Boolean {
            dir = if (isSpace(crashDir)) {
                null
            } else {
                if (crashDir.endsWith(FILE_SEP)) dir else dir!! + FILE_SEP
            }
            if (mInitialized) return true
            defaultDir = if (Environment.MEDIA_MOUNTED == Environment.getExternalStorageState() && Utils.app.externalCacheDir != null)
                Utils.app.externalCacheDir.toString() + FILE_SEP + "crash" + FILE_SEP
            else {
                Utils.app.cacheDir.toString() + FILE_SEP + "crash" + FILE_SEP
            }
