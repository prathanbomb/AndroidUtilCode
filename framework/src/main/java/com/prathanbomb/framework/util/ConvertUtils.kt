    ...
    companion object {
        ...
        fun input2OutputStream(input: InputStream?): ByteArrayOutputStream? {
            ...
            return try {
                ...
                var len: Int = input.read(b, 0, MemoryConstants.KB)
                while (input.read(b, 0, MemoryConstants.KB).let { len = it; it != -1 }) {
                    os.write(b, 0, len)
                }
                os
            } catch (e: IOException) {
        ...
        fun inputStream2String(input: InputStream?, charsetName: String): String? {
            ...
            return try {
                inputStream2Bytes(input)?.toString(charset(charsetName))
        ...
        fun outputStream2String(out: OutputStream?, charsetName: String): String? {
            ...
            return try {
                outputStream2Bytes(out)?.toString(charset(charsetName))
            } catch (e: UnsupportedEncodingException) {
                e.printStackTrace()
                null
            }
