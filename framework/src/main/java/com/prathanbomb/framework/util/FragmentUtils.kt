...
class FragmentUtils private constructor() {
    ...
    internal class Args internal constructor(var id: Int, var isHide: Boolean, var isAddStack: Boolean)
    ...
    companion object {
        ...
        private val TYPE_HIDE_ADD_FRAGMENT = 0x01 shl 1
        private val TYPE_REMOVE_FRAGMENT = 0x01 shl 2
        private val TYPE_REMOVE_TO_FRAGMENT = 0x01 shl 3
        private val TYPE_REPLACE_FRAGMENT = 0x01 shl 4
        private val TYPE_POP_ADD_FRAGMENT = 0x01 shl 5
        private val TYPE_HIDE_FRAGMENT = 0x01 shl 6
        private val TYPE_SHOW_FRAGMENT = 0x01 shl 7
        private val TYPE_HIDE_SHOW_FRAGMENT = 0x01 shl 8
         ...
         * @return fragment
         */
        fun hideAddFragment(fragmentManager: FragmentManager,
        ...
        fun addFragments(fragmentManager: FragmentManager,
                         ...
                         @IdRes containerId: Int,
                         showIndex: Int,
                         vararg lists: List<SharedElement>): Fragment? {
            var i = 0
            val size = fragments.size
            while (i < size) {
                val fragment = fragments[i]
        ...
        fun removeAllFragments(fragmentManager: FragmentManager) {
            ...
            fragments.indices.reversed()
                    .mapNotNull { fragments[it] }
                    .forEach {
                        removeAllFragments(it.childFragmentManager)
                        removeFragment(it)
                    }
        }
        ...
        fun popAllFragments(fragmentManager: FragmentManager) {
            ...
            fragments.indices.reversed()
                    .mapNotNull { fragments[it] }
                    .forEach { popAllFragments(it.childFragmentManager) }
            while (fragmentManager.backStackEntryCount > 0) {
                fragmentManager.popBackStackImmediate()
            }
        }
        ...
        private fun getTopShowFragmentIsInStack(fragmentManager: FragmentManager,
            ...
            fragments.indices.reversed()
                    ...
                    .forEach {
                        if (isInStack) {
                            if (it.arguments.getBoolean(ARGS_IS_ADD_STACK)) {
                                return getTopShowFragmentIsInStack(it.childFragmentManager, it, true)
                            }
                        } else {
                            return getTopShowFragmentIsInStack(it.childFragmentManager, it, false)
                        }
                    }
        ...
        private fun getAllFragmentsIsInStack(fragmentManager: FragmentManager,
            ...
            fragments.indices.reversed()
                    ...
                    .forEach {
                        if (isInStack) {
                            if (it.arguments.getBoolean(ARGS_IS_ADD_STACK)) {
                                result.add(FragmentNode(it, getAllFragmentsIsInStack(it.childFragmentManager, ArrayList(), true)))
                            }
                        } else {
                            result.add(FragmentNode(it, getAllFragmentsIsInStack(it.childFragmentManager, ArrayList(), false)))
                        }
                    }
            return result
        }
        ...
        fun dispatchBackPress(fragmentManager: FragmentManager): Boolean {
            ...
            return fragments.indices.reversed()
                    .map { fragments[it] }
                    .any { it != null && it.isResumed && it.isVisible && it.userVisibleHint && it is OnBackClickListener && (it as OnBackClickListener).onBackClick() }
        }
