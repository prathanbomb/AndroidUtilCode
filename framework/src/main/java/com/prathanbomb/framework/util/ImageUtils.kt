...
class ImageUtils private constructor() {
    ...
    companion object {
        ...
        fun getBitmap(@DrawableRes resId: Int, maxWidth: Int, maxHeight: Int): Bitmap {
            ...
            options.inSampleSize = calculateInSampleSize(options, maxWidth, maxHeight)
            options.inJustDecodeBounds = false
            return BitmapFactory.decodeResource(Utils.app.resources, resId, options)
        }
        ...
        fun renderScriptBlur(src: Bitmap,
                             @FloatRange(from = 0.0, to = 25.0, fromInclusive = false) radius: Float): Bitmap? {
            if (isEmptyBitmap(src)) return null
         ...
         */
        @JvmOverloads
        fun addFrame(src: Bitmap, borderWidth: Int, color: Int, recycle: Boolean = false): Bitmap? {
            if (isEmptyBitmap(src)) return null
        ...
        private fun isGIF(b: ByteArray): Boolean {
            return b.size >= 6
                    && b[0].toChar() == 'G' && b[1].toChar() == 'I'
                    && b[2].toChar() == 'F' && b[3].toChar() == '8'
                    && (b[4].toChar() == '7' || b[4].toChar() == '9') && b[5].toChar() == 'a'
        }
